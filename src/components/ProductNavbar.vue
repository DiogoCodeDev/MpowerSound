...
const goToGallery = (filter) => {
    ...
    if (route.path === '/product-gallery' && route.query.search === filter) {
        return;
    }
			...
			<div data-aos="fade-right" data-aos-delay="400"
				...
				<ul :class="!isNavbarFixed ? '' : 'mt-1'" class="flex lg:ml-12 w-11/12 justify-between">
					...
					<li :class="{ 'text-yellow-400 font-bold ': (route.path === '/produtos' || route.path.includes('/product')), 'bg-[#090909] rounded': isSubmenuOpen }"
						...
						<ul v-if="isSubmenuOpen" :class="isNavbarFixed ? 'mt-44' : 'mt-46'"
							style="background-color: #090909;"
							class="absolute w-68 shadow-md rounded mr-1 p-2 space-y-2">
							<li class="w-72 font-normal tracking-wider ml-1 pt-2 pb-1">
								<div @click="goToGallery('Caixas Amplificadoras Multi-uso')"
									class="text-white cursor-pointer hover:ml-1 font-normal tracking-wider duration-150 ease-in-out"> • &nbsp; Caixas Amplificadoras Multi-uso</div>
							</li>
							<li class="w-72 ml-1 py-1">
								<div @click="goToGallery('Amplificadores de Potência')"
									class="text-white cursor-pointer hover:ml-1 font-normal tracking-wider duration-150 ease-in-out"> • &nbsp; Amplificadores de Potência</div>
							</li>
							<li class="w-72 ml-1 py-1 pb-2">
								<div @click="goToGallery('Kits de Som Ambiente')"
									class="text-white cursor-pointer hover:ml-1 font-normal tracking-wider duration-150 ease-in-out"> • &nbsp; Kits de Som Ambiente</div>
							</li>
						</ul>
					</li>
					<li class="w-24 font-normal tracking-widest h-7 ml-3 flex items-center justify-center"
					...
					<li :class="{ 'text-yellow-400 font-bold ': route.path === '/redes', 'bg-[#090909] rounded': isSubmenuSocialMediaOpen }"
						class="w-32 h-7 font-normal tracking-widest ml-4 flex items-center text-[0.75rem] justify-center" @mouseenter="handleMouseSocialEnter"
						@mouseleave="handleMouseSocialLeave">
						<div @click="notFixeNavBar">Redes Sociais</div>
						<span class="ml-3 text-xs cursor-pointer transform transition-transform duration-300"
							:class="{ 'rotate-180': isSubmenuSocialMediaOpen }">▼</span>
						<ul v-if="isSubmenuSocialMediaOpen" :class="isNavbarFixed ? 'mt-40' : 'mt-40'"
							style="background-color: #090909;"
							...
							<transition name="slide">
								<ul v-show="isProdutosOpen" class="ml-3 mt-2 space-y-1">
									<li class="h-10 mt-4 text-[1.05rem] flex items-center text-sm">									
										<div @click="goToGallery('Caixas Amplificadoras Multi-uso')"
											class="text-white cursor-pointer hover:ml-1 font-normal tracking-wider duration-150 ease-in-out"> • &nbsp; Caixas Amplificadoras Multi-uso</div></li>
									<li class="h-10 flex text-[1.05rem] items-center text-sm"><div @click="goToGallery('Amplificadores de Potência')"
											class="text-white cursor-pointer hover:ml-1 font-normal tracking-wider duration-150 ease-in-out"> • &nbsp; Amplificadores de Potência</div></li>
									<li class="h-10 flex text-[1.05rem] items-center text-sm"><div @click="goToGallery('Kits de Som Ambiente')"
											class="text-white cursor-pointer hover:ml-1 font-normal tracking-wider duration-150 ease-in-out"> • &nbsp; Kits de Som Ambiente</div></li>
								</ul>
							</transition>
						</li>
